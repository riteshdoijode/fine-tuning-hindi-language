...
def tokenize_function(examples):
  ...
  tokenized_output = tokenizer(translations, truncation=True, padding="max_length", max_length=256, return_tensors="pt") #512 128
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    torch_dtype=torch.bfloat16,  # Forcing FP8 Change to 'bfloat16' if not working.
...
training_args = TrainingArguments(
    ...
    eval_strategy="steps", # epoch Disabled evaluation during training to save memory
    eval_steps=1000,
    save_strategy="steps",
    save_steps=500, # change for creating checkpoints
    save_total_limit=2,
    per_device_train_batch_size=4, # 4 Small batch size to fit GPU
    per_device_eval_batch_size=1, # 3 
    logging_dir="./logs",
    logging_strategy="steps",
    logging_steps=500, #10 500 100
    num_train_epochs=3, #1 2
    bf16=True,
    fp16=False,  # Set BF16 instead of FP16
    gradient_accumulation_steps=2,  #2 4 Accumulate to simulate larger batch 
    report_to="none",
